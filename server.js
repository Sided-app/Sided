...
app.use(['/api/predictions','/api/diary','/api/comments','/api/follow','/api/team-follow','/api/leagues','/api/leagues/join','/api/report','/api/block','/api/profile','/api/reactions','/api/activity','/api/leaderboard'],
...
app.get('/api/user/:uid', auth, async (req, res) => {
  ...
  const { data: prof } = await db.from('profiles').select('username,avatar,fav_team').eq('user_id', uid).single();
  ...
  res.json({ id: uid, ...prof, acc, settled: settled.length, pts });
});
