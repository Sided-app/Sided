...
const COMP_MAP = { 'World Cup':'WC','Premier League':'PL','La Liga':'PD','Serie A':'SA','Bundesliga':'BL1','Ligue 1':'FL1','Eredivisie':'DED','Brasileirão':'BSA','Champions League':'CL' };
...
const AF_LEAGUES = { 'Saudi Pro League': 307, 'MLS': 253, 'Liga MX': 262, 'Liga Argentina': 128, 'Jupiler Pro League': 144 };
...
// ── auth middleware: verify the app's Supabase JWT ──
function auth(req, res, next) {
  try { req.userId = jwt.verify((req.headers.authorization||'').replace('Bearer ',''), SUPABASE_JWT_SECRET).sub; next(); }
...
async function runSync(src = 'all') {
  ...
  if (src !== 'af') for (const code of Object.values(COMP_MAP)) {
    ...
    for (const m of data.matches || []) {
      const row = { id:m.id, competition:code, home_team:m.homeTeam.name, away_team:m.awayTeam.name, utc_date:m.utcDate, status:m.status, home_score:m.score?.fullTime?.home ?? null, away_score:m.score?.fullTime?.away ?? null, matchday:m.matchday ?? null, updated_at:new Date().toISOString() };
  ...
  if (src !== 'fd' && process.env.APIFOOTBALL_TOKEN) {
    for (const [name, id] of Object.entries(AF_LEAGUES)) {
      ...
      for (const m of data.response || []) {
        ...
        const row = { id: AF_OFFSET + m.fixture.id, competition: name, home_team: m.teams.home.name, away_team: m.teams.away.name, utc_date: m.fixture.date, status, home_score: m.goals.home, away_score: m.goals.away, matchday: null, updated_at: new Date().toISOString() };
        await db.from('fixtures').upsert(row); upserted++;
